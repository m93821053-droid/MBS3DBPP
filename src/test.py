...
def outputResult2(packingResult1, packingResult2, bin_size=100):
    '''
    packingResult: [box1,box2,box3,...]
    box1:[l,w,h,p_x,p_y,p_z]
    '''
    # step1: check the correct of outputData: whether any box overlap with other box
    box_num = len(packingResult1)
    for i in range(box_num - 1):
        for j in range(i + 1, box_num):
            box_i = np.array(packingResult1[i])
            box_j = np.array(packingResult1[j])
            box_i_c = box_i[0:3] / 2 + box_i[3:]
            box_j_c = box_j[0:3] / 2 + box_j[3:]
            is_overlap = (np.abs(box_j_c - box_i_c) < (box_i[0:3] + box_j[0:3]) / 2).all()
            if is_overlap:
    ...
    # step2: calculate the use rate
    packingArray = np.array(packingResult1)
    boxHeightCo = packingArray[:, 2] + packingArray[:, 5]
    max_height1 = np.max(boxHeightCo)
    ur1 = packingArray[:, :3].prod(1).sum() / bin_size / bin_size / max_height1
    # step2: calculate the use rate
    packingArray = np.array(packingResult2)
    boxHeightCo = packingArray[:, 2] + packingArray[:, 5]
    max_height2 = np.max(boxHeightCo)
    ur2 = packingArray[:, :3].prod(1).sum() / bin_size / bin_size / max_height2
    ...
    ax.set_xlim(0, bin_size)
    ax.set_ylim(0, bin_size)
    ax.set_zlim(0, max_height1)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title("Best:%.2s%%" % (ur1.item() * 100), fontproperties="SimHei")
    for box in packingResult1:
        color = (random.random(), random.random(), random.random(), 1)
        length, width, height, start_l, start_w, start_h = box
        plotBox(start_l, start_w, start_h,
                length, width, height, ax, color)
    ...
    ax.set_xlim(0, bin_size)
    ax.set_ylim(0, bin_size)
    ax.set_zlim(0, max_height2)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title("Worse:%.2s%%" % (ur2.item() * 100), fontproperties="SimHei")
    for box in packingResult2:
        color = (random.random(), random.random(), random.random(), 1)
        length, width, height, start_l, start_w, start_h = box
        plotBox(start_l, start_w, start_h,
                length, width, height, ax, color)
...
if __name__ == '__main__':
    ...
    # parameter
    box_num = 50
    for i, bin_type_list in enumerate([[(50, 50, 50), (80, 80, 80), (100, 100, 100), (120, 120, 120)],
                                       [(50, 50, 50), (80, 80, 80), (100, 100, 100), (120, 120, 120), (50, 80, 100)]]):
        sample_num = 16
        test_num = 1024
        box_num = box_num
        bin_size_x = 100
        ...
        plane_feature_num = 6
        gap_filling = False
        load_file_path = r"./save/100_100_100_10_10_50_1_5_0_0_"+str(i+4)+"_None_2_50_0/actor.pth"
        ...
        with h5py.File(r"./datasets/test_dataset" + str(box_num) + ".h5", 'r') as f:
            box_array_list = f['arrays'][:]

        # box_array_list = [Env.generate_box_array(bin_size_x, bin_size_y, bin_size_z, box_num, min_factor, max_factor, box_set=None) for _ in
        #                  range(test_num)]

        # box_array_list = [Env.generate_box_array(bin_size_x, bin_size_y, box_num, 0.2, 0.2, box_set=None) for
        #                   _ in
        #                   range(test_num)]
        ...
        act = ActorNetwork(bin_size_ds_x,bin_size_ds_y, box_num, bin_type_list, online,plane_feature_num=plane_feature_num,binary_dim=10).to(device)
        ...
        for i in range(test_num):
            print(i,flush=True)
            print("Time:" + str(total_time),flush=True)
            ...
            for j in range(box_num):
                ...
                if result_list[2][0]:
                    ...
                    for i in range(5):
                        ur_list[i].append(max(result_list[3][:2**i]))
                    packing_result_list.append(result_list[4][np.array(result_list[3]).argmax()])
                    total_time += time1
                    break

        use_ratio = [sum(ur_list[i]) / test_num for i in range(5)]
        use_time = total_time / test_num
        #print(ur_list)
        print("average use ratio:", use_ratio, flush=True)
        print("average time:", use_time, flush=True)
        ...
        #     if len(bestResult[i]) != 0:
        #         plotResult(bestResult[i], bin_size_x,bin_size_y,bin_size_z)
        # print("The best use ratio of instances is:%.2f%%" % (max(ur_list)))
        # print("worseResult")
        # for i in range(len(worseResult)):
        #     if len(worseResult[i]) != 0:
        #         plotResult(worseResult[i], bin_size_x, bin_size_y, bin_size_z)
