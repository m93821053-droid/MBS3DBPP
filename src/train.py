...
def train_and_evaluate(args, action_queue_list, result_queue_list):
    ...
    agent = AgentPPO(bin_size_x=args.bin_size_ds_x,
                     ...
                     bin_type_list=args.bin_type_list,
                     online=args.online,
    ...
    while evaluator.total_step < break_step:
        time_s1 = time.time()
        with torch.no_grad():
            trajectory_list = agent.explore_env_mp(action_queue_list, result_queue_list, target_step, reward_scale, gamma)
            ur = trajectory_list[-1]
            steps, r_exp = update_buffer(trajectory_list[:-1])
            evaluator.total_step += steps
            evaluator.save_model(agent.act,agent.cri,agent.goal_avg)
        print("===========================================================",flush=True)
        print("The time of explore environment: %s" % (int(time.time() - time_s1)), flush=True)
        print("The use ratio: %s" % (ur), flush=True)
        time_s2 = time.time()

        # logging_tuple: [critic_loss,actor_loss,logprob]
        logging_tuple = agent.update_net(buffer, batch_size, repeat_times, soft_update_tau,trunc_step)

        print("The time of update_net: %s" % (int(time.time() - time_s2)), flush=True)

        fr = open(f'{cwd}/result_ur.txt', 'a')
        fr.write(str(evaluator.total_step) + '\t' + str(ur) +'\n')
        fr.close()
...
if __name__ == '__main__':
    ...
    process_list = list()
    if args.box_set_num is not None:
        if args.box_set_num == "paper":
            print("generate box set refer to paper.")
            temp_list = []
            for i in range(2,6):
                for j in range(2,6):
                    for k in range(2,6):
                        temp_list.append([i,j,k])
            box_set = np.array(temp_list)
        elif args.box_set_num == "cut1":
            box_set = "cut_1"
        else:
            box_set = Env.generate_box_array(args.bin_size_x,args.bin_size_y, args.bin_size_z, args.box_set_num, args.min_factor, args.max_factor)
